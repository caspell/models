...
train_checkpoint = '/home/mhkim/data/checkpoint/mnist_cnn_scan/'
...
def main () :
    ...
    W1 = tf.Variable(tf.truncated_normal([IMAGE_SIZE, 5, NUM_CHANNELS, 24], stddev=0.1, seed=SEED, dtype=tf.float32), name='W1')
    b1 = tf.Variable(tf.zeros([24], dtype=tf.float32), name='bias1')

    W2 = tf.Variable(tf.truncated_normal([5, 5, 24, 64], stddev=0.1, seed=SEED, dtype=tf.float32), name='W2')
    b2 = tf.Variable(tf.constant(0.1, shape=[64], dtype=tf.float32), name='bias2')

    # W3 = tf.Variable(tf.truncated_normal([5, 5, 64, 64], stddev=0.1, seed=SEED, dtype=tf.float32), name='W3')
    # b3 = tf.Variable(tf.constant(0.1, shape=[64], dtype=tf.float32), name='bias3')
    ...
    with tf.name_scope('model') :
        ...
        # conv = tf.nn.conv2d(pool, W3, strides=[1, 1, 1, 1], padding='SAME')
